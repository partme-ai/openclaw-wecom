...
export async function handleInboundMessage(
  ...
  const result = streamStore.addPendingMessage({
    ...
    nonce,
    timestamp,
    debounceMs: (target.account.botConfig as any)?.debounceMs
  });
  ...
  streamStore.updateStream(streamId, (s) => {
    ...
    s.aibotid = target.account.botConfig?.botId;
  });

  // 根据 status 返回不同的占位符响应
  const defaultPlaceholder = target.account.streamPlaceholderContent;
